...
public class HanziToPinyin {
    ...
    public static class Token {
        ...
        public Token(int type, String source, String target, int position) {
            this.type = type;
            this.source = source;
            this.target = target;
...
//                offset = (begin + end) / 2;
    ...
    //增加对空格和大写的处理。
    public ArrayList<Token> getSort(final String input) {
        ArrayList<Token> tokens = new ArrayList<Token>();
        if (!mHasChinaCollator || TextUtils.isEmpty(input)) {
            // return empty tokens.
            return tokens;
        ...
        for (int i = 0; i < inputLength; i++) {
            ...
            if (character == ' ') {
                if (sb.length() > 0) {
                    sb.append(character);
                    space = true;
//                    addToken(sb, tokens, tokenType, startPosition);
                }
            } else if (character < 256) {
     //           if (tokenType != Token.LATIN && sb.length() > 0) {
	            if ((tokenType != Token.LATIN || Character.isUpperCase(character) || space)&& sb.length() > 0) {
	                space = false;
	                addToken(sb, tokens, tokenType, startPosition);
                }
                tokenType = Token.LATIN;
                ...
                if ((tokenType != Token.UNKNOWN || space)&& sb.length() > 0 ) {
                    space = false;
                    addToken(sb, tokens, tokenType, startPosition);
                }
                tokenType = Token.UNKNOWN;
                ...
                if (t.type == Token.PINYIN) {
                    ...
                    if ((tokenType != t.type || space) && sb.length() > 0) {
                        space = false;
                        addToken(sb, tokens, tokenType, startPosition);
                    }
                    tokenType = t.type;
    ...
    private void addToken(
            final StringBuilder sb, final ArrayList<Token> tokens, final int tokenType, int position) {
        String str = sb.toString();
        tokens.add(new Token(tokenType, str, str, position));
        sb.setLength(0);
    }
    
    public HashSet<String> getNameLookupKeys(String name) {
        // TODO : Reduce the object allocation.
        HashSet<String> keys = new HashSet<String>();
        ArrayList<Token> tokens = getSort(name);
...
//            Log.d(TAG, "keyOrignal = " + keyOrignal.toString() + "keyPinyin = " +keyPinyin.toString() + "keyInitial = " + keyInitial.toString());

//            oldType = token.type;
        }
        
//        keys.add(keyOrignal.toString().toUpperCase());
        keys.add(keyPinyin.toString().toUpperCase());
        keys.add(keyInitial.toString().toUpperCase());
        
//        if (!keyOrignal.toString().toUpperCase().equals(name.toUpperCase()))
//        {
...
//        }
    ...
    public boolean includeLookUpKey(String s, Iterator<String> lookupKey)
    {
        String info = null;
        while (lookupKey.hasNext()) {
            info = lookupKey.next();
            if (info.contains(s)) {
                return true;
            }

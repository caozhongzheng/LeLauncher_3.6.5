...
//	Intent intent;
    ...
    public ItemInfo(ItemInfo info) {
        ...
        attachedIndexArray = info.attachedIndexArray;
        
        icon = info.icon;
        uri = info.uri;
        commendView = info.commendView;
        minSpanX = info.minSpanX;
        minSpanY = info.minSpanY;
        isGesture = info.isGesture;
        dropPos = info.dropPos;
    }
     ...
     * set bitmap to values to be saved
     */
    public static void writeReplaceBitmap(ContentValues values, Bitmap bitmap) {
        if (bitmap != null) {
            byte[] data = flattenBitmap(bitmap);
            values.put(LauncherSettings.Favorites.ICON_REPLACE, data);
        } else {
            byte[] data = null;
            values.put(LauncherSettings.Favorites.ICON_REPLACE, data);
        }

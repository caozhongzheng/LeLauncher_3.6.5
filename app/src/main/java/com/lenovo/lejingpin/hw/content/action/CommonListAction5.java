...
import com.lenovo.lejingpin.ams.CommInfoRequest5.CommInfo;
import com.lenovo.lejingpin.ams.CommInfoRequest5.CommInfoResponse5;
...
public class CommonListAction5 implements Action {
	private String TAG = "CommonListAction";
//	private Context mContext;
	private static String mPackageName;
	private static String mVersionCode;
	private static CommonListAction5 mAction;

	private CommonListAction5() {
	}

	private CommonListAction5(String packageName, String versionCode) {
//		mContext = context;
	}

	public static CommonListAction5 getInstance(String packageName, String versioncode) {
		if(mAction == null) {
			mAction = new CommonListAction5(packageName, versioncode);
		}
	...
	private void getCommonList(final Context context, String packageName, String versionCode) {
		CommInfoRequest5 request = new CommInfoRequest5(context);
		...
		AmsSession5.execute(context, request, new AmsCallback() {
			@Override
			public void onResult(AmsRequest request, int code, byte[] bytes) {
				CommInfoResponse5 response = new CommInfoResponse5();
				if(bytes != null) {
				...
				if(commonList != null) {
					for (CommInfo info : commonList) {
						...
						builder.append(info.getUser_nick());
						builder.append("@%!");
						builder.append(info.getComment_context());
						builder.append("@%!");
						builder.append(info.getComment_date());
	...
	private void getCommentList_HawaiiSearch(final Context context, String packageName, String versionCode) {
		...
		AmsSession.execute(context, request, new AmsSession.AmsCallback() {
			@Override
			public void onResult(AmsRequest request, int code, byte[] bytes) {
				...
				if( success){
				    ArrayList<CommInfoRequest.CommInfo> commonList = response.getItemList();
				    commonStringList = new ArrayList<String>();
				    if(commonList != null) {
					    for (CommInfoRequest.CommInfo info : commonList) {
						    StringBuilder builder = new StringBuilder();

...
public class NewSearchAppName implements AmsRequest {
	...
	public NewSearchAppName(int padOrPhone){
		if(padOrPhone==-1){
			multiple = 20;
			searchCount = "20";
		}else{
	...
	public void setStartIndex (int startindex){
		si = Integer.toString(startindex*multiple);
	}
	...
	public String getUrl() {
		...
		String url =null;
		try {
//			ams1001834efbe248cc4f95adfec556f3e50af6
			url = AmsSession.sAmsRequestHost + "ams/3.0/appopensearch.do"
					+ "?type=" + URLEncoder.encode("1", "UTF-8") 
					+ "&clientid="+URLEncoder.encode(RegistClientInfoResponse.getClientId(), "UTF-8")
					+ "&si="+ URLEncoder.encode(si, "UTF-8") 
					+"&cnt="+ URLEncoder.encode("20", "UTF-8") 
					+"&kw="+ kw 
					+ "&from="+17016
					+"&stf=" + URLEncoder.encode("appname", "UTF-8")
					+"&srf="+URLEncoder.encode("uuid,lcaid,appid,apppack,appname,publishdate,ispay,appprice,appversion,appsize,appaddr,iconaddr,addv,star,download,click,appversioncode", "UTF-8")
					+"&order="+URLEncoder.encode("score+desc,click+desc", "UTF-8")
					+"&hl="+0
...
//		String url ;
//		url = AmsSession.sAmsRequestHost + "ams/3.0/appopensearch.do"
//				+ "?type=" + 2 + "&clientid="+RegistClientInfoResponse.getClientId()+ 
//				"&si="+ 0 +"&cnt="+ 10+"&kw="+ kw + "&stf=" + "appname"+
//				"&srf="+"uuid,lcaid,appid,apppack,appname,publishdate,ispay,appprice,appversion,appsize,appaddr,iconaddr,addv,star,download,click"
//				+"&order="+"score+desc,click+desc";
//		
//		
		Log.i("yangmao", "get url is:"+url);
		return url;
...
//			ams1001834efbe248cc4f95adfec556f3e50af6
			url = AmsSession.sAmsRequestHost + "ams/3.0/appopensearch.do"
					...
					//+ "clientid="+URLEncoder.encode(RegistClientInfoResponse.getClientId(), "UTF-8")
					//+ "&clientid="+URLEncoder.encode("ams1001834efbe248cc4f95adfec556f3e50af6", "UTF-8")
					+ "?si="+ URLEncoder.encode(si, "UTF-8") 
					+"&cnt="+ URLEncoder.encode("20", "UTF-8") 
					+"&kw="+ kw 
					+ "&from="+17016
					+"&stf=" + URLEncoder.encode("appname", "UTF-8")
					+"&srf="+URLEncoder.encode("lcaid,appid,apppack,appname,publishdate,ispay,appprice,appversion,appsize,appaddr,iconaddr,addv,star,download,click,appversioncode", "UTF-8")
					+"&order="+URLEncoder.encode("score+desc,click+desc", "UTF-8")
					+"&hl="+0 
					+"&ispay="+0;
	...
	*/
	...
	public String getUrl() {
		...
		try {
			...
			url = AmsSession.sAmsRequestHost + "ams/3.0/appopensearch.do"					 	
					+ "?si="+ URLEncoder.encode(si, "UTF-8") 
					+"&cnt="+ URLEncoder.encode(searchCount, "UTF-8") 
					+"&kw="+ kw 
					+ "&from="+17016
					+"&stf=" + URLEncoder.encode("appname", "UTF-8")
					+"&srf="+URLEncoder.encode("lcaid,appid,apppack,appname,publishdate,ispay,appprice,appversion,appsize,appaddr,iconaddr,addv,star,download,click,appversioncode", "UTF-8")
	...
	public static final class NewSearchResponse implements AmsResponse {
		...
		public void parseFrom(byte[] bytes) {
			...
			try {
				...
				Log.i("xiaoyangmao", "the number found is:"+result.getString("numFound"));										
				...
				if (jsArray.length() != 0) {
					for (int i = 0; i < jsArray.length(); i++) {
						...
						application.setApp_size(jsonObject2.getString("appsize"));
						application.setLcaId(jsonObject2.getString("lcaid"));
						application.setPackage_name(jsonObject2.getString("apppack"));
						application.setApp_versioncode(jsonObject2.getString("appversioncode"));
						application.setApp_version(jsonObject2.getString("appversion"));
						application.setStar_level(jsonObject2.getString("star"));
						application.setIsPay(jsonObject2.getString("ispay"));
						application.setApp_price(jsonObject2.getString("appprice"));
						mApplications.add(application);
						
						//yangmao add new to zdx interface
						
//						RecommendLocalAppInfo application_ym = new RecommendLocalAppInfo();
...
//						mApplications_ym.add(application_ym);
				...
				mIntent.setAction("com.lenovo.lejingpin.newsearch.exception");

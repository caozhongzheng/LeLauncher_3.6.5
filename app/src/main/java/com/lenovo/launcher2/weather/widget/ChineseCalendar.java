package com.lenovo.launcher2.weather.widget;

import com.lenovo.launcher.R;

import java.util.Calendar;
import java.util.Date;
import java.util.GregorianCalendar;


import android.content.Context;
import android.content.res.Resources;
import android.text.TextUtils;


public class ChineseCalendar extends GregorianCalendar {
	private static final long serialVersionUID = 8L;

	public static final int CHINESE_YEAR = 801;

	public static final int CHINESE_MONTH = 802;

	public static final int CHINESE_DATE = 803;

	public static final int CHINESE_SECTIONAL_TERM = 804;

	public static final int CHINESE_PRINCIPLE_TERM = 805;

	public static final int CHINESE_HEAVENLY_STEM = 806;

	public static final int CHINESE_EARTHLY_BRANCH = 807;

	public static final int CHINESE_ZODIAC = 808;

	public static final int CHINESE_TERM_OR_DATE = 888;
	
	public static final int CHINESE_FESITIVALS = 800;
	
	protected static final String CY="年",CM="月",CD="日",CYM="闰",CNL="农历"; 

	private int chineseYear;
	private int chineseMonth;
	private int chineseDate;
	private int sectionalTerm;
	private int principleTerm;

	private boolean areChineseFieldsComputed;
	private boolean areSolarTermsComputed;
	private boolean lastSetChinese;
	
	private static final int baseYear = 1901;
	private static final int baseMonth = 1;
	private static final int baseDate = 1;
	private static final int baseIndex = 0;
	private static final int baseChineseYear = 1900;
	private static final int baseChineseMonth = 11;
	private static final int baseChineseDate = 11;
	
	
	private static String[] chineseWeekNames = new String[8];
	private static String[] chineseMonthNames = new String[13];
	private static String[] chineseDateNames = new String[31];
	private static String[] principleTermNames = new String[12];
	private static String[] sectionalTermNames = new String[12];
	private static String[] stemNames = new String[13];
	private static String[] branchNames = new String[13];
	private static String[] chinseFesitivals = new String[10];
	private static String[] newFesitivals = new String[4];
	
	Resources res;

	public ChineseCalendar(Context mContext) {
		super();
		res = mContext.getResources();
		String[] chineseString = null;
        chineseString=res.getStringArray(R.array.animalNames);
        if(TextUtils.isEmpty(chineseString[0]))
        	chineseString[0]="";
        setAnimalNames(chineseString);
        
        
        chineseString=res.getStringArray(R.array.branchNames);
        if(TextUtils.isEmpty(chineseString[0]))
        	chineseString[0]="";
        setBranchNames(chineseString);
        
        chineseString=res.getStringArray(R.array.chineseDateNames);
        if(TextUtils.isEmpty(chineseString[0]))
        	chineseString[0]="";
        setChineseDateNames(chineseString);
        
        chineseString=res.getStringArray(R.array.chineseMonthNames);
        if(TextUtils.isEmpty(chineseString[0]))
        	chineseString[0]="";
        setChineseMonthNames(chineseString);
        
        chineseString=res.getStringArray(R.array.chineseWeekNames);
        if(TextUtils.isEmpty(chineseString[0]))
        	chineseString[0]="";
        setChineseWeekNames(chineseString);
        
        chineseString=res.getStringArray(R.array.principleTermNames);
        if(TextUtils.isEmpty(chineseString[0]))
        	chineseString[0]="";
        setPrincipleTermNames(chineseString);
        
        chineseString=res.getStringArray(R.array.sectionalTermNames);
        if(TextUtils.isEmpty(chineseString[0]))
        	chineseString[0]="";
        setSectionalTermNames(chineseString);
        
        chineseString=res.getStringArray(R.array.stemNames);
        if(TextUtils.isEmpty(chineseString[0]))
        	chineseString[0]="";
        setStemNames(chineseString);
        
        chineseString=res.getStringArray(R.array.Festival);
        if(TextUtils.isEmpty(chineseString[0]))
        	chinseFesitivals[0]="";
        setFesitivals(chineseString);
        
        chineseString=res.getStringArray(R.array.calendarFestival);
        if(TextUtils.isEmpty(chineseString[0]))
        	newFesitivals[0]="";
        setNewFesitivals(chineseString);
	}

	public ChineseCalendar(Date d) {
		super.setTime(d);
	}

	public ChineseCalendar(Calendar c) {
		this(c.getTime());
	}

	public ChineseCalendar(int y, int m, int d) {
		super(y, m, d);
	}

	public ChineseCalendar(boolean isChinese, int y, int m, int d) {
		if (isChinese) {
			setChnDate(CHINESE_YEAR, y);
			setChnDate(CHINESE_MONTH, m);
			setChnDate(CHINESE_DATE, d);
		} else {
			set(y, m, d);
		}
	}

	private void setChnDate(int field, int value) {
		computeIfNeed(field);

		if (isChineseField(field)) {

			switch (field) {
			case CHINESE_YEAR:
				chineseYear = value;
				break;
			case CHINESE_MONTH:
				chineseMonth = value;
				break;
			case CHINESE_DATE:
				chineseDate = value;
				break;
			default:
				throw new IllegalArgumentException("no found field：" + field);
			}
			lastSetChinese = true;
		} else {

			super.set(field, value);
			lastSetChinese = false;
		}
		areFieldsSet = false;
		areChineseFieldsComputed = false;
		areSolarTermsComputed = false;
	}

	public int get(int field) {
		computeIfNeed(field);

		if (!isChineseField(field)) {
			return super.get(field);
		}

		int ret = -1;
		switch (field) {
		case CHINESE_YEAR:
			ret = chineseYear;
			break;
		case CHINESE_MONTH:
			ret = chineseMonth;
			break;
		case CHINESE_DATE:
			ret = chineseDate;
			break;
		case CHINESE_SECTIONAL_TERM:
			ret = sectionalTerm;
			break;
		case CHINESE_PRINCIPLE_TERM:
			ret = principleTerm;
			break;
		case CHINESE_HEAVENLY_STEM:
			ret = (chineseYear - 4) % 10 + 1;
			break;
		case CHINESE_EARTHLY_BRANCH:
		case CHINESE_ZODIAC:
			ret = (chineseYear - 4) % 12 + 1;
			break;
		case CHINESE_FESITIVALS:
			if(getFesitivals() != null){
				ret = CHINESE_FESITIVALS;
			}
			ret = CHINESE_FESITIVALS+100;
			break;
		case CHINESE_TERM_OR_DATE:
			int option;
//			Log.e("1111111111","get(Calendar.DATE) ====== "+get(Calendar.DATE));
			if (get(Calendar.DATE) == get(CHINESE_SECTIONAL_TERM)) {
				option = CHINESE_SECTIONAL_TERM;
			} else if (get(Calendar.DATE) == get(CHINESE_PRINCIPLE_TERM)) {
				option = CHINESE_PRINCIPLE_TERM;
			} else if (get(CHINESE_DATE) == 1) {
				option = CHINESE_MONTH;
			} else if(get(CHINESE_FESITIVALS)== CHINESE_FESITIVALS){
				option = CHINESE_FESITIVALS;
//				Log.e("111111","CHINESE_FESITIVALS = ======== "+CHINESE_FESITIVALS+"---option=="+option);
			}else{
				option = CHINESE_DATE;
			}
			ret = option;
			break;
		default:
			throw new IllegalArgumentException("no found field：" + field);
		}
		return ret;
	}

	public void add(int field, int amount) {
		computeIfNeed(field);

		if (!isChineseField(field)) {
			super.add(field, amount);
			lastSetChinese = false;
			areChineseFieldsComputed = false;
			areSolarTermsComputed = false;
			return;
		}

		switch (field) {
		case CHINESE_YEAR:
			chineseYear += amount;
			break;
		case CHINESE_MONTH:
			for (int i = 0; i < amount; i++) {
				chineseMonth = nextChineseMonth(chineseYear, chineseMonth);
				if (chineseMonth == 1) {
					chineseYear++;
				}
			}
			break;
		case CHINESE_DATE:
			int maxDate = daysInChineseMonth(chineseYear, chineseMonth);
			for (int i = 0; i < amount; i++) {
				chineseDate++;
				if (chineseDate > maxDate) {
					chineseDate = 1;
					chineseMonth = nextChineseMonth(chineseYear, chineseMonth);
					if (chineseMonth == 1) {
						chineseYear++;
					}
					maxDate = daysInChineseMonth(chineseYear, chineseMonth);
				}
			}
			break;
		default:
			throw new IllegalArgumentException("no found field：" + field);
		}

		lastSetChinese = true;
		areFieldsSet = false;
		areChineseFieldsComputed = false;
		areSolarTermsComputed = false;
	}

	public void roll(int field, int amount) {
		computeIfNeed(field);

		if (!isChineseField(field)) {
			super.roll(field, amount);
			lastSetChinese = false;
			areChineseFieldsComputed = false;
			areSolarTermsComputed = false;
			return;
		}

		switch (field) {
		case CHINESE_YEAR:
			chineseYear += amount;
			break;
		case CHINESE_MONTH:
			for (int i = 0; i < amount; i++) {
				chineseMonth = nextChineseMonth(chineseYear, chineseMonth);
			}
			break;
		case CHINESE_DATE:
			int maxDate = daysInChineseMonth(chineseYear, chineseMonth);
			for (int i = 0; i < amount; i++) {
				chineseDate++;
				if (chineseDate > maxDate) {
					chineseDate = 1;
				}
			}
			break;
		default:
			throw new IllegalArgumentException("no found field：" + field);
		}

		lastSetChinese = true;
		areFieldsSet = false;
		areChineseFieldsComputed = false;
		areSolarTermsComputed = false;
	}

	public String getChinese(int field) {
		computeIfNeed(field);

		String ret = "";
		switch (field) {
		case CHINESE_FESITIVALS:
			ret = getFesitivals();
			break;
		case CHINESE_YEAR:
			ret = getChinese(CHINESE_HEAVENLY_STEM)
					+ getChinese(CHINESE_EARTHLY_BRANCH) + CY;//cy=年
			break;
		case CHINESE_MONTH:
			if (chineseMonth > 0)
				ret = chineseMonthNames[chineseMonth] + CM;//cm=月
			else
				ret = CYM + chineseMonthNames[-chineseMonth] + CM;//ym=润.cm=月
			break;
		case CHINESE_DATE:
			ret = chineseDateNames[chineseDate];
			break;
		case CHINESE_SECTIONAL_TERM:
			ret = sectionalTermNames[get(Calendar.MONTH)];
			break;
		case CHINESE_PRINCIPLE_TERM:
			ret = principleTermNames[get(Calendar.MONTH)];
			break;
		case CHINESE_HEAVENLY_STEM:
			ret = stemNames[get(field)];
			break;
		case CHINESE_EARTHLY_BRANCH:
			ret = branchNames[get(field)];
			break;
		case CHINESE_ZODIAC:
			ret = animalNames[get(field)];
			break;
		case Calendar.DAY_OF_WEEK:
			ret = chineseWeekNames[get(field)];
			break;
		case CHINESE_TERM_OR_DATE:
			ret = getChinese(get(CHINESE_TERM_OR_DATE));
			break;
		default:
			throw new IllegalArgumentException("bad chinese field：" + field);
		}
		return ret;
	}

	public String getSimpleGregorianDateString() {
		return new StringBuffer().append(get(YEAR)).append("-")
				.append(get(MONTH) + 1).append("-").append(get(DATE))
				.toString();
	}

	public String getFesitivals(){
		String chineseMoon = getChinese(CHINESE_MONTH);
		String chineseData  = getChinese(CHINESE_DATE);
		if(chineseMoon.contains(chineseMonthNames[1])){
			if(chineseData.contains(chineseDateNames[1])){
				return chinseFesitivals[0];//春节
			}else if(chineseData.contains(chineseDateNames[15])){
				return chinseFesitivals[1];//元宵节
			};
		}
		if(chineseMoon.contains(chineseMonthNames[5])){
			if(chineseData.contains(chineseDateNames[5])){
				return chinseFesitivals[2];//端午节
			}
		}
		if(chineseMoon.contains(chineseMonthNames[7])){
			if(chineseData.contains(chineseDateNames[7])){
				return chinseFesitivals[3];//七夕节
			}else if(chineseData.contains(chineseDateNames[15])){
				return chinseFesitivals[4];//中元节
			}
		}
		if(chineseMoon.contains(chineseMonthNames[8])){
			if(chineseData.contains(chineseDateNames[15])){
				return chinseFesitivals[5];//中秋节
			}
		}
		if(chineseMoon.contains(chineseMonthNames[9])){
			if(chineseData.contains(chineseDateNames[9])){

				return chinseFesitivals[6];//重阳节
			}
		}
		if(chineseMoon.contains(chineseMonthNames[12])){
			if(chineseData.contains(chineseDateNames[8])){
				return chinseFesitivals[7];//腊八节
			}
		}
		if(chineseMoon.contains(chineseMonthNames[12])){
			if(chineseData.contains(chineseDateNames[30])){
				return chinseFesitivals[8];//除夕
			}
		}
		int month = get(Calendar.MONTH);
		int data  = get(Calendar.DATE);
		if(month == 0){
			if(data == 1){
				return newFesitivals[0];//元旦
			}
		}
		if(month == 1){
			if(data == 14){
				return newFesitivals[1];//情人节
			}
		}
	
		if(month == 2){
			if(data == 8){
				return newFesitivals[2];//妇女节  
			}
		}
		if(month == 4){
			if(data == 1){
				return newFesitivals[3];//劳动节
			}
		}
		if(month == 5){
			if(data == 1){
				return newFesitivals[4];//儿童节
			}
		}
		if(month == 7){
			if(data == 1){
				return newFesitivals[7];//建军节
			}
		}
		if(month == 8){
			if(data == 10){
				return newFesitivals[5];//教师节
			}
		}
		if(month == 9){
			if(data == 1){
				return newFesitivals[6];//国庆节
			}
		}
		
		return null;
	}
	
	public String getSimpleChineseDateString() {
		return new StringBuffer()
				.append(get(CHINESE_YEAR))
				.append("-")
				.append(get(CHINESE_MONTH) > 0 ? "" + get(CHINESE_MONTH) : "*"
						+ (-get(CHINESE_MONTH))).append("-")
				.append(get(CHINESE_DATE)).toString();
	}

	public String getChineseDateString() {
		return new StringBuffer().append(getChinese(CHINESE_YEAR)+" ")
				.append(getChinese(CHINESE_MONTH)+" ")
				.append(getChinese(CHINESE_DATE)).toString();
	}

	public String toString() {
		StringBuffer buf = new StringBuffer();
		buf.append(getSimpleGregorianDateString()).append(" | ")
				.append(getChinese(DAY_OF_WEEK)).append(" | ["+CNL+"]")//农历
				.append(getChineseDateString()).append(" ")
				.append(getChinese(CHINESE_ZODIAC)).append(CY).append(" ")//cy=年
				.append(get(CHINESE_SECTIONAL_TERM)).append(CD)//cd=日
				.append(getChinese(CHINESE_SECTIONAL_TERM)).append(" ")
				.append(get(CHINESE_PRINCIPLE_TERM)).append(CD)//cd=日
				.append(getChinese(CHINESE_PRINCIPLE_TERM));
		return buf.toString();
	}

	private boolean isChineseField(int field) {
		boolean isChineseField = false;
		switch (field) {
		case CHINESE_YEAR:
		case CHINESE_MONTH:
		case CHINESE_DATE:
		case CHINESE_SECTIONAL_TERM:
		case CHINESE_PRINCIPLE_TERM:
		case CHINESE_HEAVENLY_STEM:
		case CHINESE_EARTHLY_BRANCH:
		case CHINESE_ZODIAC:
		case CHINESE_TERM_OR_DATE:
		case CHINESE_FESITIVALS:
			isChineseField = true;
			break;
		default:
			isChineseField = false;
		}
		return isChineseField ;
	}

	private boolean isChineseTermsField(int field) {
		boolean isChineseTermsField = false;
		switch (field) {
		case CHINESE_SECTIONAL_TERM:
		case CHINESE_PRINCIPLE_TERM:
		case CHINESE_TERM_OR_DATE:
			isChineseTermsField =  true;
			break;
		default:
			isChineseTermsField = false;
			break;
		}
		return isChineseTermsField;
	}

	private void computeIfNeed(int field) {
		if (isChineseField(field)) {
			if (!lastSetChinese && !areChineseFieldsComputed) {
				super.complete();
				computeChineseFields();
				areFieldsSet = true;
				areChineseFieldsComputed = true;
				areSolarTermsComputed = false;
			}
			if (isChineseTermsField(field) && !areSolarTermsComputed) {
				computeSolarTerms();
				areSolarTermsComputed = true;
			}
		} else {
			if (lastSetChinese && !areFieldsSet) {
				computeGregorianFields();
				super.complete();
				areFieldsSet = true;
				areChineseFieldsComputed = true;
				areSolarTermsComputed = false;
			}
		}
	}

	private void computeGregorianFields() {
		int y = chineseYear;
		int m = chineseMonth;
		int d = chineseDate;
		areChineseFieldsComputed = true;
		areFieldsSet = true;
		lastSetChinese = false;

		if (y < 1900)
			y = 1899;
		else if (y > 2100)
			y = 2101;

		if (m < -12)
			m = -12;
		else if (m > 12)
			m = 12;

		if (d < 1)
			d = 1;
		else if (d > 30)
			d = 30;

		int dateint = y * 10000 + Math.abs(m) * 100 + d;
		if (dateint < 19001111) {
			set(1901, Calendar.JANUARY, 1);
			super.complete();
		} else if (dateint > 21001201) {
			set(2100, Calendar.DECEMBER, 31);
			super.complete();
		} else {
			if (Math.abs(m) > 12) {
				m = 12;
			}
			int days = ChineseCalendar.daysInChineseMonth(y, m);
			if (days == 0) {
				m = -m;
				days = ChineseCalendar.daysInChineseMonth(y, m);
			}
			if (d > days) {
				d = days;
			}
			set(y, Math.abs(m) - 1, d);
			computeChineseFields();

			int amount = 0;
			while (chineseYear != y || chineseMonth != m) {
				amount += daysInChineseMonth(chineseYear, chineseMonth);
				chineseMonth = nextChineseMonth(chineseYear, chineseMonth);
			}
			amount += d - chineseDate;

			super.add(Calendar.DATE, amount);
		}
		computeChineseFields();
	}

	private void computeChineseFields() {
		int gregorianYear = internalGet(Calendar.YEAR);
		int gregorianMonth = internalGet(Calendar.MONTH) + 1;
		int gregorianDate = internalGet(Calendar.DATE);

		if (gregorianYear < 1901 || gregorianYear > 2100) {
			return;
		}

		int startYear = baseYear;
		int startMonth = baseMonth;
		int startDate = baseDate;
		chineseYear = baseChineseYear;
		chineseMonth = baseChineseMonth;
		chineseDate = baseChineseDate;

		if (gregorianYear >= 2000) {
			startYear = baseYear + 99;
			startMonth = 1;
			startDate = 1;
			chineseYear = baseChineseYear + 99;
			chineseMonth = 11;
			chineseDate = 25;
		}
		int daysDiff = 0;
		for (int i = startYear; i < gregorianYear; i++) {
			daysDiff += 365;
			if (isGregorianLeapYear(i)) {
				daysDiff += 1;
			}
		}
		for (int i = startMonth; i < gregorianMonth; i++) {
			daysDiff += daysInGregorianMonth(gregorianYear, i - 1);
		}
		daysDiff += gregorianDate - startDate;

		chineseDate += daysDiff;
		int lastDate = daysInChineseMonth(chineseYear, chineseMonth);
		int nextMonth = nextChineseMonth(chineseYear, chineseMonth);
		while (chineseDate > lastDate) {
			if (Math.abs(nextMonth) < Math.abs(chineseMonth)) {
				chineseYear++;
			}
			chineseMonth = nextMonth;
			chineseDate -= lastDate;
			lastDate = daysInChineseMonth(chineseYear, chineseMonth);
			nextMonth = nextChineseMonth(chineseYear, chineseMonth);
		}

	}

	private void computeSolarTerms() {
		int gregorianYear = internalGet(Calendar.YEAR);
		int gregorianMonth = internalGet(Calendar.MONTH);

		if (gregorianYear < 1901 || gregorianYear > 2100) {
			return;
		}
		sectionalTerm = sectionalTerm(gregorianYear, gregorianMonth);
		principleTerm = principleTerm(gregorianYear, gregorianMonth);
	}

	public static boolean isGregorianLeapYear(int year) {
		boolean isLeap = false;
		if (year % 4 == 0) {
			isLeap = true;
		}
		if (year % 100 == 0) {
			isLeap = false;
		}
		if (year % 400 == 0) {
			isLeap = true;
		}
		return isLeap;
	}

	public static int daysInGregorianMonth(int y, int m) {
		int d = daysInGregorianMonth[m];
		if (m == Calendar.FEBRUARY && isGregorianLeapYear(y)) {
			d++;
		}
		return d;
	}

	public static int sectionalTerm(int y, int m) {
		m++;
		if (y < 1901 || y > 2100) {
			return 0;
		}
		int index = 0;
		int ry = y - baseYear + 1;
		while (ry >= sectionalTermYear[m - 1][index]) {
			index++;
		}
		int term = sectionalTermMap[m - 1][4 * index + ry % 4];
		if ((ry == 121) && (m == 4)) {
			term = 5;
		}
		if ((ry == 132) && (m == 4)) {
			term = 5;
		}
		if ((ry == 194) && (m == 6)) {
			term = 6;
		}
		return term;
	}

	public static int principleTerm(int y, int m) {
		m++;
		if (y < 1901 || y > 2100) {
			return 0;
		}
		int index = 0;
		int ry = y - baseYear + 1;
		while (ry >= principleTermYear[m - 1][index]) {
			index++;
		}
		int term = principleTermMap[m - 1][4 * index + ry % 4];
		if ((ry == 171) && (m == 3)) {
			term = 21;
		}
		if ((ry == 181) && (m == 5)) {
			term = 21;
		}
		return term;
	}

	public static int daysInChineseMonth(int y, int m) {

		int index = y - baseChineseYear + baseIndex;
		int v = 0;
		int l = 0;
		int d = 30;
		if (1 <= m && m <= 8) {
			v = chineseMonths[2 * index];
			l = m - 1;
			if (((v >> l) & 0x01) == 1) {
				d = 29;
			}
		} else if (9 <= m && m <= 12) {
			v = chineseMonths[2 * index + 1];
			l = m - 9;
			if (((v >> l) & 0x01) == 1) {
				d = 29;
			}
		} else {
			v = chineseMonths[2 * index + 1];
			v = (v >> 4) & 0x0F;
			if (v != Math.abs(m)) {
				d = 0;
			} else {
				d = 29;
				for (int i = 0; i < bigLeapMonthYears.length; i++) {
					if (bigLeapMonthYears[i] == index) {
						d = 30;
						break;
					}
				}
			}
		}
		return d;
	}

	public static int nextChineseMonth(int y, int m) {
		int n = Math.abs(m) + 1;
		if (m > 0) {
			int index = y - baseChineseYear + baseIndex;
			int v = chineseMonths[2 * index + 1];
			v = (v >> 4) & 0x0F;
			if (v == m) {
				n = -m;
			}
		}
		if (n == 13) {
			n = 1;
		}
		return n;
	}

	

	public static void setChineseWeekNames(String[] chWeekNames) {
		chineseWeekNames = chWeekNames;
	}

	

	public static void setChineseMonthNames(String[] chMonthNames) {
		chineseMonthNames = chMonthNames;
	}

	

	public static void setChineseDateNames(String[] chDateNames) {
		chineseDateNames = chDateNames;
	}

	

	public static void setPrincipleTermNames(String[] prinmNames) {
		principleTermNames = prinmNames;
	}

	

	public static void setSectionalTermNames(String[] sectTermNames) {
		sectionalTermNames = sectTermNames;
	}

	
	public static void setFesitivals(String[] stem) {
		chinseFesitivals = stem;
	}
	public static void setNewFesitivals(String[] stem) {
		newFesitivals = stem;
	}
	
	public static void setStemNames(String[] stem) {
		stemNames = stem;
	}

	

	public static void setBranchNames(String[] branch) {
		branchNames = branch;
	}


	public static void setAnimalNames(String[] animal) {
		animalNames = animal;
	}

	private static String[] animalNames = new String[13];

	private static final int[] bigLeapMonthYears = { 6, 14, 19, 25, 33, 36, 38,
			41, 44, 52, 55, 79, 117, 136, 147, 150, 155, 158, 185, 193 };
	private static final char[][] sectionalTermMap = {
			{ 7, 6, 6, 6, 6, 6, 6, 6, 6, 5, 6, 6, 6, 5, 5, 6, 6, 5, 5, 5, 5, 5,
					5, 5, 5, 4, 5, 5 },
			{ 5, 4, 5, 5, 5, 4, 4, 5, 5, 4, 4, 4, 4, 4, 4, 4, 4, 3, 4, 4, 4, 3,
					3, 4, 4, 3, 3, 3 },
			{ 6, 6, 6, 7, 6, 6, 6, 6, 5, 6, 6, 6, 5, 5, 6, 6, 5, 5, 5, 6, 5, 5,
					5, 5, 4, 5, 5, 5, 5 },
			{ 5, 5, 6, 6, 5, 5, 5, 6, 5, 5, 5, 5, 4, 5, 5, 5, 4, 4, 5, 5, 4, 4,
					4, 5, 4, 4, 4, 4, 5 },
			{ 6, 6, 6, 7, 6, 6, 6, 6, 5, 6, 6, 6, 5, 5, 6, 6, 5, 5, 5, 6, 5, 5,
					5, 5, 4, 5, 5, 5, 5 },
			{ 6, 6, 7, 7, 6, 6, 6, 7, 6, 6, 6, 6, 5, 6, 6, 6, 5, 5, 6, 6, 5, 5,
					5, 6, 5, 5, 5, 5, 4, 5, 5, 5, 5 },
			{ 7, 8, 8, 8, 7, 7, 8, 8, 7, 7, 7, 8, 7, 7, 7, 7, 6, 7, 7, 7, 6, 6,
					7, 7, 6, 6, 6, 7, 7 },
			{ 8, 8, 8, 9, 8, 8, 8, 8, 7, 8, 8, 8, 7, 7, 8, 8, 7, 7, 7, 8, 7, 7,
					7, 7, 6, 7, 7, 7, 6, 6, 7, 7, 7 },
			{ 8, 8, 8, 9, 8, 8, 8, 8, 7, 8, 8, 8, 7, 7, 8, 8, 7, 7, 7, 8, 7, 7,
					7, 7, 6, 7, 7, 7, 7 },
			{ 9, 9, 9, 9, 8, 9, 9, 9, 8, 8, 9, 9, 8, 8, 8, 9, 8, 8, 8, 8, 7, 8,
					8, 8, 7, 7, 8, 8, 8 },
			{ 8, 8, 8, 8, 7, 8, 8, 8, 7, 7, 8, 8, 7, 7, 7, 8, 7, 7, 7, 7, 6, 7,
					7, 7, 6, 6, 7, 7, 7 },
			{ 7, 8, 8, 8, 7, 7, 8, 8, 7, 7, 7, 8, 7, 7, 7, 7, 6, 7, 7, 7, 6, 6,
					7, 7, 6, 6, 6, 7, 7 } };
	private static final char[][] sectionalTermYear = {
			{ 13, 49, 85, 117, 149, 185, 201, 250, 250 },
			{ 13, 45, 81, 117, 149, 185, 201, 250, 250 },
			{ 13, 48, 84, 112, 148, 184, 200, 201, 250 },
			{ 13, 45, 76, 108, 140, 172, 200, 201, 250 },
			{ 13, 44, 72, 104, 132, 168, 200, 201, 250 },
			{ 5, 33, 68, 96, 124, 152, 188, 200, 201 },
			{ 29, 57, 85, 120, 148, 176, 200, 201, 250 },
			{ 13, 48, 76, 104, 132, 168, 196, 200, 201 },
			{ 25, 60, 88, 120, 148, 184, 200, 201, 250 },
			{ 16, 44, 76, 108, 144, 172, 200, 201, 250 },
			{ 28, 60, 92, 124, 160, 192, 200, 201, 250 },
			{ 17, 53, 85, 124, 156, 188, 200, 201, 250 } };
	private static final char[][] principleTermMap = {
			{ 21, 21, 21, 21, 21, 20, 21, 21, 21, 20, 20, 21, 21, 20, 20, 20,
					20, 20, 20, 20, 20, 19, 20, 20, 20, 19, 19, 20 },
			{ 20, 19, 19, 20, 20, 19, 19, 19, 19, 19, 19, 19, 19, 18, 19, 19,
					19, 18, 18, 19, 19, 18, 18, 18, 18, 18, 18, 18 },
			{ 21, 21, 21, 22, 21, 21, 21, 21, 20, 21, 21, 21, 20, 20, 21, 21,
					20, 20, 20, 21, 20, 20, 20, 20, 19, 20, 20, 20, 20 },
			{ 20, 21, 21, 21, 20, 20, 21, 21, 20, 20, 20, 21, 20, 20, 20, 20,
					19, 20, 20, 20, 19, 19, 20, 20, 19, 19, 19, 20, 20 },
			{ 21, 22, 22, 22, 21, 21, 22, 22, 21, 21, 21, 22, 21, 21, 21, 21,
					20, 21, 21, 21, 20, 20, 21, 21, 20, 20, 20, 21, 21 },
			{ 22, 22, 22, 22, 21, 22, 22, 22, 21, 21, 22, 22, 21, 21, 21, 22,
					21, 21, 21, 21, 20, 21, 21, 21, 20, 20, 21, 21, 21 },
			{ 23, 23, 24, 24, 23, 23, 23, 24, 23, 23, 23, 23, 22, 23, 23, 23,
					22, 22, 23, 23, 22, 22, 22, 23, 22, 22, 22, 22, 23 },
			{ 23, 24, 24, 24, 23, 23, 24, 24, 23, 23, 23, 24, 23, 23, 23, 23,
					22, 23, 23, 23, 22, 22, 23, 23, 22, 22, 22, 23, 23 },
			{ 23, 24, 24, 24, 23, 23, 24, 24, 23, 23, 23, 24, 23, 23, 23, 23,
					22, 23, 23, 23, 22, 22, 23, 23, 22, 22, 22, 23, 23 },
			{ 24, 24, 24, 24, 23, 24, 24, 24, 23, 23, 24, 24, 23, 23, 23, 24,
					23, 23, 23, 23, 22, 23, 23, 23, 22, 22, 23, 23, 23 },
			{ 23, 23, 23, 23, 22, 23, 23, 23, 22, 22, 23, 23, 22, 22, 22, 23,
					22, 22, 22, 22, 21, 22, 22, 22, 21, 21, 22, 22, 22 },
			{ 22, 22, 23, 23, 22, 22, 22, 23, 22, 22, 22, 22, 21, 22, 22, 22,
					21, 21, 22, 22, 21, 21, 21, 22, 21, 21, 21, 21, 22 } };
	private static final char[][] principleTermYear = {
			{ 13, 45, 81, 113, 149, 185, 201 },
			{ 21, 57, 93, 125, 161, 193, 201 },
			{ 21, 56, 88, 120, 152, 188, 200, 201 },
			{ 21, 49, 81, 116, 144, 176, 200, 201 },
			{ 17, 49, 77, 112, 140, 168, 200, 201 },
			{ 28, 60, 88, 116, 148, 180, 200, 201 },
			{ 25, 53, 84, 112, 144, 172, 200, 201 },
			{ 29, 57, 89, 120, 148, 180, 200, 201 },
			{ 17, 45, 73, 108, 140, 168, 200, 201 },
			{ 28, 60, 92, 124, 160, 192, 200, 201 },
			{ 16, 44, 80, 112, 148, 180, 200, 201 },
			{ 17, 53, 88, 120, 156, 188, 200, 201 } };

	private static final char[] daysInGregorianMonth = { 31, 28, 31, 30, 31,
			30, 31, 31, 30, 31, 30, 31 };
	private static final char[] chineseMonths = {  };

}

...
public class CacheProvider extends ContentProvider {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + CacheFiles.TAB_NAME + " ( "
					...
					+ CacheFiles.URL + " TEXT UNIQUE ON CONFLICT REPLACE," + CacheFiles.POSTDATA
					+ " TEXT," + CacheFiles.FILENAME + " TEXT,"
					+ CacheFiles.LASTACCESSTIME + " TEXT,"
	...
	public int delete(Uri uri, String where, String[] selectionArgs) {
		int count = 0;
		if (URL_MATCHER.match(uri) == CACHE) {
	...
	public Uri insert(Uri url, ContentValues initialValues) {
		...
		Uri uri = null;
		if (URL_MATCHER.match(url) == CACHE) {
			SQLiteDatabase db = mDbHelper.getWritableDatabase();
	...
	static {
		...
		URL_MATCHER.addURI(CacheFiles.CONTENT_URI_STRING, CacheFiles.TAB_NAME,
				CACHE);
	}

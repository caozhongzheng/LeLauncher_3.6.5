...
public class NetworkHttpRequest {
	...
	public HttpReturn executeHttpGet(Context context, String url,
		...
		Log.i(TAG, "Request time consuming = " + (endTime - startTime) / 1000
				+ "sec");
		return ret;
	}
	...
	public HttpReturn executeHttpPost(Context context, String url, String post,
		...
		try {
			...
			Log.i(TAG, "NetworkHttpRequest >> executeHttpPost >> post : "
					+ post);
			Log.i(TAG, "Request start time:" + startTime);
			HttpParams params = new BasicHttpParams();
			HttpConnectionParams.setConnectionTimeout(params, DEFAULTTIMEOUT);
			...
			if (requestFrom == 1) {
				...
				/*
				 * httppost.addHeader( "Cookie", "lpsust=" +
				 * PsAuthenServiceL.getStData(context, SnsRequest.RID, visiable)
				 * + ";clientid=" + SnsRegistClientInfoResponse.getClientId());
				 */
			}

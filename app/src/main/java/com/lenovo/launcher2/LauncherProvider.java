...
public class LauncherProvider extends ContentProvider {
    ...
    /*RK_ID: RK_TRASH . AUT: zhanggx1 . DATE: 2012-04-10 . S*/
//    private static final int DATABASE_VERSION = 10;//old is 9
//    private static final int DATABASE_VERSION = 11; // for hawaii by liuli1, 2012-06-01
    private static final int DATABASE_VERSION = 13; // for app's category by liuli1, 2012-06-15
    /*RK_ID: RK_TRASH . AUT: zhanggx1 . DATE: 2012-04-10 . E*/
    ...
    static final Uri CONTENT_APPWIDGET_RESET_URI =
            Uri.parse("content://" + Constants.AUTHORITY + "/appWidgetReset");
    
    private DatabaseHelper mOpenHelper;

    /** AUT: henryyu1986@163.com DATE: 2011-12-20 S */
    private static final String INIT_DEFAULT_FAVORITE_WIDGET = "init_default_favorite_widget";
    /** AUT: henryyu1986@163.com DATE: 2011-12-20 E */
    @Override
    ...
    public int bulkInsert(Uri uri, ContentValues[] values) {
        ...
        try {
            ...
            Log.v("SQLiteDiskIOException","  insert database error ");
            db.endTransaction();
            return 0;
        }finally {
            db.endTransaction();
    ...
    public int bulkInsertApplication(ContentValues[] values) {
        SqlArguments args = new SqlArguments(LauncherSettings.Applications.CONTENT_URI);
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        /*** AUT: zhaoxy . DATE: 2012-08-21. START***/
        ...
        /*** AUT: zhaoxy . DATE: 2012-08-21. END***/
        db.beginTransaction();
        db.execSQL("DELETE FROM " + Constants.TABLE_APPLICATIONS);
        try {
            int numValues = values.length;
            if (args.table.equals(Constants.TABLE_APPLICATIONS)) {
                for (int i = 0; i < numValues; i++) {
        ...
        sendNotify(LauncherSettings.Applications.CONTENT_URI);
        return values.length;
    }
    ...
    private void sendNotify(Uri uri) {
        String notify = uri.getQueryParameter(Constants.PARAMETER_NOTIFY);
    ...
    /*
     * generate new _id for TABLE_APPLICATIONS
     */
    public long generateNewAppsId() {
        return mOpenHelper.generateNewAppsId();
    }
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        private static final String TAG_COMMENT = "commend";
        ...
        DatabaseHelper(Context context) {
            ...
            mContext = context;
            mAppWidgetHost = new AppWidgetHost(context, XLauncher.APPWIDGET_HOST_ID);
            ...
            /* AUT: liuli liuli0120@use.com.cn DATE: 2012-01-31 START */
            if (mMaxAppsId == -1) {
                mMaxAppsId = initializeMaxAppsId(getWritableDatabase());
            }
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE favorites (" +
                    ...
                    "lastUseTime INTEGER NOT NULL DEFAULT " +  System.currentTimeMillis() + "," + 
            ...
            db.execSQL("CREATE TABLE " + Constants.TABLE_APPLICATIONS + " (" +
                    "_id INTEGER PRIMARY KEY," +
                    LauncherSettings.Applications.LABEL + " TEXT," +
                    LauncherSettings.Applications.CLASS + " TEXT," +
                    LauncherSettings.Applications.CELL_INDEX + " INTEGER, " +
                    LauncherSettings.Applications.APP_CAN_DRAG + " INTEGER NOT NULL DEFAULT 1" +
                    ");");
        ...
        private int copyFromCursor(SQLiteDatabase db, Cursor c) {
            ...
            try {
                ...
                for (i = 0; i < numValues; i++) {
                    if (dbInsertAndCheck(this, db, Constants.TABLE_FAVORITES, null, rows[i]) < 0) {
                        return 0;
                    } else {
                        total++;
                    }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (version < 9) {
                ...
                /* AUT: liuli liuli0120@use.com.cn DATE: 2012-01-31 START */
                if (mMaxAppsId == -1) {
                    mMaxAppsId = initializeMaxAppsId(db);
                }
                /* AUT: liuli liuli0120@use.com.cn DATE: 2012-01-31 END */
            ...
            if (version < 10) {
	            ...
	            try {
	                // Insert new column for holding appWidgetIds
	                db.execSQL("ALTER TABLE favorites " +
	                    "ADD COLUMN lastUseTime INTEGER NOT NULL DEFAULT " + System.currentTimeMillis() + ";");
	                db.execSQL("ALTER TABLE favorites " +
	                	"ADD COLUMN needConfig INTEGER DEFAULT 0;");
	                db.setTransactionSuccessful();
	                version = 10;
	            } catch (SQLException ex) {
	                // Old version remains, which means we wipe old data
            ...
            if (version != DATABASE_VERSION) {
                ...
                /* AUT: liuli liuli0120@use.com.cn DATE: 2012-01-31 START */
                db.execSQL("DROP TABLE IF EXISTS " + Constants.TABLE_APPLICATIONS);
//                // for apps category manager
                db.execSQL("DROP TABLE IF EXISTS " + TABLE_CATEGORY);
                db.execSQL("DROP TABLE IF EXISTS " + TABLE_CATEGORY_MAP);
               /* AUT: liuli liuli0120@use.com.cn DATE: 2012-01-31 END */
        ...
        /*RK_ID: RK_UPDATE . AUT: zhanggx1 . DATE: 2012-05-23 . E*/
        @Override
        ...
        private boolean updateContactsShortcuts(SQLiteDatabase db) {
            ...
            try {
                ...
                c = db.query(Constants.TABLE_FAVORITES, new String[] { Favorites._ID, Favorites.INTENT },
                    ...
                    if (intentUri != null) {
                        try {
                            ...
                            if (Intent.ACTION_VIEW.equals(intent.getAction()) &&
                                ...
                                db.update(Constants.TABLE_FAVORITES, values, updateWhere, null);                                
                            }
        ...
        private long initializeMaxAppsId(SQLiteDatabase db) {
            Cursor c = db.rawQuery("SELECT MAX(_id) FROM " + Constants.TABLE_APPLICATIONS, null);
        ...
        private void convertWidgets(SQLiteDatabase db) {
            ...
            try {
                // Select and iterate through each matching widget
                c = db.query(Constants.TABLE_FAVORITES, new String[] { Favorites._ID, Favorites.ITEM_TYPE },
                        selectWhere, null, null, null, null);
        ...
        private int loadFavorites(SQLiteDatabase db, long filterContainerId) {
        	/** ID: scene backup & restore. AUT: chengliang . DATE: 2012.04.10 S */
/* RK_ID:RK_BOOT_POLICY_DIALOG. AUT:liuyg1 . DATE:2013-3-21. START*/ 
//			try {
//				R2.echo("Perform factory restore from provider !");
//				byte res = com.lenovo.launcher2.backup.BackupManager.getInstance(mContext)
//						.performDefaultRestore();
//				if (res == OperationState.CRITICAL_DEFAULT_RESTORING_NEED_START)
//					return 1;
//			} catch (Exception e1) {
//				return 1;
//			}
/* RK_ID:RK_BOOT_POLICY_DIALOG. AUT:liuyg1 . DATE:2013-3-21. END*/ 
			/** ID: scene backup & restore. AUT: chengliang . DATE: 2012.04.10 E */
...
//                        }
        ...
        private long addAppShortcut(SQLiteDatabase db, ContentValues values, TypedArray a,
            ...
            try {
                ...
                if (dbInsertAndCheck(this, db, Constants.TABLE_FAVORITES, null, values) < 0) {
                    return -1;
                }
        ...
        private boolean addAppWidget(SQLiteDatabase db, ContentValues values, TypedArray a,
            ...
            if (hasPackage) {
                ...
                intent.setComponent(cn);
                values.put(Favorites.INTENT, intent.toUri(0));
            	values.put(Favorites.ITEM_TYPE, Favorites.ITEM_TYPE_COMMEND_APPWIDGET);
            }
...
//                values.put(Favorites.ITEM_TYPE, Favorites.ITEM_TYPE_APPWIDGET);
                ...
                values.put(Favorites.APPWIDGET_ID, appWidgetId);
                values.put(Favorites._ID, generateNewId());
                dbInsertAndCheck(this, db, Constants.TABLE_FAVORITES, null, values);

                allocatedAppWidgets = true;
                
                /** AUT: henryyu1986@163.com DATE: 2011-12-29 S */
                if(values.getAsInteger(Favorites.ITEM_TYPE) == Favorites.ITEM_TYPE_APPWIDGET) {
    ...
    /*RK_ID: RK_CUSTOM_WORKSPACE . AUT: zhanggx1 . DATE: 2012-12-19 . S*/
    ...
    public void transferScreens(int index) {
    	StringBuffer sb = new StringBuffer("update favorites set screen = (screen - 1) where screen > ");
    	sb.append(index)
    	  .append(" and container == -100;");
        executeSql(sb.toString());
//        sendNotify(LauncherSettings.Favorites.CONTENT_URI);
    }
    
    public void changeScreenOrder(int fromIndex, int toIndex) {
    	if (fromIndex == toIndex) {
    		return;
    	}
    	boolean fromGreater = fromIndex > toIndex;
    	String startIndex = fromGreater ? Integer.toString(toIndex) : Integer.toString(fromIndex);
    	String endIndex = fromGreater ? Integer.toString(fromIndex) : Integer.toString(toIndex);
    	String result = fromGreater ? " (screen + 1) " : " (screen - 1) ";
    	
    	StringBuffer sb = new StringBuffer("update favorites set screen = case when screen == ");
    	sb.append(Integer.toString(fromIndex))
    	  .append(" then ")
    	  .append(Integer.toString(toIndex))
    	  .append(" else ")
    	  .append(result)
    	  .append(" end where screen >= ")
    	  .append(startIndex)
    	  .append(" and screen <= ")
    	  .append(endIndex)
    	  .append(" and container == -100;");
...
//        sendNotify(LauncherSettings.Favorites.CONTENT_URI);
    ...
    /*RK_ID: RK_CUSTOM_WORKSPACE . AUT: zhanggx1 . DATE: 2012-12-19 . E*/
}

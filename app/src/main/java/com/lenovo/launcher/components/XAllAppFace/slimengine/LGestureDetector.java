...
public class LGestureDetector {
	...
	public static class SimpleOnGestureListener implements OnGestureListener,
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2,
				float distanceX, float distanceY, float previousX, float previousY) {
			return false;
		}
	...
	public static float mScrollYfactor = 1;
	...
	private final int DOUBLE_TAP_TIMEOUT = ViewConfiguration
			.getDoubleTapTimeout();
	
	public static void setLongPressTimeOut( int timeout ){
		LONGPRESS_TIMEOUT = timeout;
	}
	
	public static void updateLongPressTimeout() {
//		LONGPRESS_TIMEOUT = ViewConfiguration.getLongPressTimeout();
		if (mDebug)R5.echo("LONGPRESS_TIMEOUT = " + LONGPRESS_TIMEOUT);
	}
	 ...
	 */
	@Deprecated
	public LGestureDetector(OnGestureListener listener, Handler handler) {
	 ...
	 */
	@Deprecated
	public LGestureDetector(OnGestureListener listener) {
...
//			touchSlop = ViewConfiguration.getTouchSlop();
			...
			mNormalFlingVelocity = 6 * mMinimumFlingVelocity;
		} else {
			final ViewConfiguration configuration = ViewConfiguration
					.get(context);
//			touchSlop = configuration.getScaledTouchSlop();
			mToucheSlop = (int)(8 * context.getResources().getDisplayMetrics().density);
			...
			mNormalFlingVelocity = (int) (300 * context.getResources().getDisplayMetrics().density);
		}				
		
//		R5.echo("context = " + context + "mMinimumFlingVelocity = " + mMinimumFlingVelocity 
//		        + "mMaximumFlingVelocity = " + mMaximumFlingVelocity);

		mTouchSlopSquare = mToucheSlop * mToucheSlop;
		mScrollYfactor = (float)Math.tan(55 * Math.PI / 180);
		R5.echo("mScrollYfactor = " + mScrollYfactor);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if (mDebug)R5.echo("LGestureDetector onTouchEvent = " + ev.getAction());
		...
		case MotionEvent.ACTION_DOWN:
			...
			mTwoFinger = false;
			mPreviousMotionX = mLastMotionX = x;
			mPreviousMotionY = mLastMotionY = y;
			if (mCurrentDownEvent != null) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mInLongPress || (mIgnoreMultitouch && ev.getPointerCount() > 1 ) || mTwoFinger) {
				if (mDebug) R5.echo("mInLongPress = " + mInLongPress);
				break;
			}
			...
			if (mIsDoubleTapping) {
				...
				if (distance > mTouchSlopSquare) {
					if (mDebug)R5.echo("deltaX = " + deltaX + "deltaY = " + deltaY
							+ "ev.getPointerCount() = " + ev.getPointerCount());
					
					//如果是双指且方向相反，走双指流程。
					if (ev.getPointerCount() == 2 && !xLauncher.isSameOrientation(ev))
					{
						if(mDebug)R5.echo("two finger");
						cancel();
//						xLauncher.setBlockEvent(true);
						mTwoFinger = true;
						return false;
					}
					else if (ev.getPointerCount() >= 2 && (Math.abs(deltaY) > mScrollYfactor * Math.abs(deltaX)))
					{
						if(mDebug)R5.echo("two move");
						cancel();
						mTwoFinger = true;
						return false;
					}
					...
					handled = mListener.onScroll(mCurrentDownEvent, ev,
							scrollX, scrollY, mPreviousMotionX - x, mPreviousMotionY - y);
					mLastMotionX = x;
					mLastMotionY = y;
					mAlwaysInTapRegion = false;
					mHandler.removeMessages(TAP);
			...
			} else if ((Math.abs(scrollX) >= 0) || (Math.abs(scrollY) >= 0)) {
				handled = mListener.onScroll(mCurrentDownEvent, ev, scrollX,
						scrollY, mPreviousMotionX - x, mPreviousMotionY - y);
				mLastMotionX = x;
				mLastMotionY = y;
			}
		...
		case MotionEvent.ACTION_UP:
			...
			if (mIsDoubleTapping) {
				...
				if(mDebug)R5.echo("LGestureDetector onSingleTapUp");
...
//				    R5.echo("velocityY = " + velocityY + "velocityX = " + velocityX);
	...
	{
	    return mNormalFlingVelocity;
	}
	
	public void setOffsetWhenSizeChanged(float offsetX, float offsetY) {
		mPreviousMotionX = mLastMotionX = mLastMotionX + offsetX;
		mPreviousMotionY = mLastMotionY = mLastMotionY + offsetY;
		if (mCurrentDownEvent != null)
		{
			mCurrentDownEvent.setLocation(mLastMotionX, mLastMotionY);
		}
	}
